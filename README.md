...
### Отчёт (детально, шаблон)

Параметры прогона:

| Параметр | Значение |
|---|---|
| Окружение | docker-compose (локально) |
| Дата/время | 2026‑02‑02 12:00 UTC+3 |
| Версия | commit `HEAD` |
| Набор данных | 2 пользователя, 40 кадров (jpg) |
| Порог | `FACE_MATCH_THRESHOLD=0.55` |

Функциональные кейсы (API):

| ID | Сценарий | Эндпоинт | Ожидаемо | Статус | Примечание |
|---|---|---|---|---|---|
| F‑01 | Получить эмбеддинг | `POST /auth/face/capture` | 200 + embedding[] | PASS | ok |
| F‑02 | Регистрация | `POST /auth/face/register-with-embeddings` | 200 + user + tokens | PASS | ok |
| F‑03 | Вход | `POST /auth/face/login-with-embeddings` | 200 + user + tokens | PASS | ok |
| F‑04 | Вход с неверным лицом | `POST /auth/face/login-with-embeddings` | 401 | PASS | отказ корректный |
| F‑05 | Неверный payload | `POST /auth/face/capture` | 400 | PASS | валидация |
| F‑06 | Слишком большой кадр | `POST /auth/face/capture` | 413 | PASS | обработано на UI |

Нагрузочные сценарии:

| Сценарий | Профиль | Входные данные | Длительность |
|---|---|---|---|
| Capture | 10 VU, constant | base64 кадры | 3 мин |
| Login | 5 VU, constant | 3‑5 embeddings | 3 мин |
| Register | 2 VU, constant | 6‑8 embeddings | 2 мин |

Метрики (пример):

| Сценарий | RPS | p50 | p95 | p99 | Ошибки | CPU gateway |
|---|---|---|---|---|---|---|
| Capture | 14.6 | 210 ms | 520 ms | 780 ms | 0.6% | 48% |
| Login | 6.2 | 120 ms | 240 ms | 360 ms | 0% | 22% |
| Register | 2.1 | 180 ms | 410 ms | 610 ms | 0% | 18% |

Примечание: значения в таблицах — шаблон отчёта. После реального прогона заменяются на фактические цифры.
